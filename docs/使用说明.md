...
> 前置：内网机需已安装 **JRE/JDK 8+**（JPype 需要）与 **Node.js/pm2**。
...
| `username` / `password` | 视驱动 | 支持 `{ENV:VAR}` 从环境变量注入，**避免明文入库** |
...
**Q8：JVM 相关报错？**
确认已安装 JRE/JDK 8+。JPype 需要可用的 JVM。
